...
/*   Updated: 2024/11/11 16:34:10 by jslusark         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void add_token(t_cmd **cmd, t_mock *mock_token)
{

	t_list	*node;

	if (!(*cmd)) // if cmd had not a first token
	{
		* = node; // we add a token to the p
		return ;
	}
...
// gpt
    ...
    // Assuming t_list is a linked list structure with ft_lstadd_back
    ft_lstadd_back(&cmd->tokens, ft_lstnew(new_token));
}

t_cmd *get_cmd(t_mock *mock_token)
{
	t_cmd *cmd;
	...
	//unsure if assigning null is needed
	cmd->tokens = NULL;
    cmd->redirs = NULL;
	while (mock_token && mock_token->mock_type != PIPE) // pipe is the delimiter in this case
	{
		// if ((*mock_token)->mock_type == REDIR_IN ||
		// 		(*mock_token)->mock_type == REDIR_OUT ||
		// 		(*mock_token)->mock_type == APPEND_OUT ||
		// 		(*mock_token)->mock_type == HEREDOC)
		// {
		// 	t_redir *redir = get_redir(mock_token); // need to check what this is
		// 	add_to_list(&cmd->redirections, ft_lstnew(redir)); // this list addto front first then back
		// }
		else
		{
			add_token(cmd, mock_token);
		}
	}
	return cmd;
}
...
/*
Noticed that my approach in node creation is similar
to this project:
ref https://github.com/DimitriDaSilva/42_minishell/blob/master/src/parse/parse.c#L64
*/
...
// {
//     t_node *node;
//     t_command_node *sequence; <-- is this a linked list??
