...
/*   Updated: 2025/01/28 13:42:15 by jslusark         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (sorted_env[i] != NULL)
	{
		...
		while (sorted_env[i][j] != '\0')
		{
			if (sorted_env[i][j] == '=' && !inside_value)
			{
				...
				ft_putchar_fd('"', 1);             // Add a quote after '='
				inside_value = true;
			}
...
void exec_export(char **av, t_node_list *node)
{
	if (av == NULL)
	{
		...
		// node->msh->exit_code = 0; // took out as i would update it at the begining of the main.c loop
		// return;
	}
	else if (ft_strchr(av[0], '=') == NULL) // would do this differently to check edge cases
	{
		write(1, "implement add to matrix as empty variable\n", 42);
		// node->msh->exit_code = 0;
		return;
	}
	else // make export checks before doing this
